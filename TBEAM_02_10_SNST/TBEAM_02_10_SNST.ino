...
const String firmwareVersion = "1.2.02.10.18:00";  // Cambia esta versión según corresponda
...
void mostrarAyuda() {
    // Mostrar ayuda en la consola serial
    ...
    Serial.println("log on         - Activa la visualización de logs detallados.");
    Serial.println("log off        - Desactiva la visualización de logs detallados.");
    ...
    Serial.println("version        - Muestra la versión actual del firmware.");
    Serial.println("help           - Muestra esta ayuda.");

    // Mostrar ayuda en el terminal de Blynk
    ...
    terminal.println("log on         - Activa la visualización de logs detallados.");
    terminal.println("log off        - Desactiva la visualización de logs detallados.");
    ...
    terminal.println("version        - Muestra la versión actual del firmware.");
    terminal.println("help           - Muestra esta ayuda.");
    terminal.flush();
}
...
void processCommand(const String& command) {
    // Mostrar siempre en la consola serial
    printBoth("Comando recibido: " + command);

    // Mostrar siempre en el terminal de Blynk
    ...
    if (command == "reboot") {
        ...
        Serial.println("Versión actual del firmware:");
        Serial.println(firmwareVersion);

        terminal.println("Versión actual del firmware:");
    ...
    // Realizamos un flush al final para enviar todos los mensajes acumulados
    terminal.flush();
}
...
bool enviaDatosGPS() {
    ...
    for (int i = 0; i < 3; i++) {  // Intentar enviar hasta 3 veces
        ...
        esp_task_wdt_reset();  // Reiniciar el Watchdog Timer para evitar el tiempo de espera
        if (client.publish("sensorDevice/data", payload.c_str(), qos)) {
            ...
            delay(100);  // Agregar un pequeño delay para permitir al sistema operativo manejar tareas críticas
            esp_task_wdt_reset();  // Reiniciar el Watchdog Timer para evitar el tiempo de espera
        }
    }
...
void sincronizaTiempo() {
  // Configura los servidores NTP
  configTime(0, 0, "pool.ntp.org", "time.nist.gov");
  
  // Configura la zona horaria para Chile con el offset correcto
  setenv("TZ", "CLT+4CLST,M09.1.0,M04.1.0", 1);
  tzset();
