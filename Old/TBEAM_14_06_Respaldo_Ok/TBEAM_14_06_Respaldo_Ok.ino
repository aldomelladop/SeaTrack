...
void enviaDatosGPS() {
    ...
    if (!client.connected()) {
        Serial.println("Error al publicar los datos en AWS IoT");
    } else {
        Serial.println("Datos publicados en AWS IoT exitosamente");
    }
}
...
void muestraEstadoBateria() {
    ...
    Serial.print("USB Connected: "); Serial.println(usbConnected ? "YES" : "NO");
...
//         float distance = gps.distanceBetween(filteredLat, filteredLng, ports[i].lat, ports[i].lng);
//         if (distance <= ports[i].radius) {
//             String portQuery = String("?idDevice=") + deviceID +
//                                "&idCentro=" + ports[i].name +
//                                "&Fecha=" + generaJSONPayload() +
//                                "&Lat=" + String(filteredLat, 6) +
//                                "&Long=" + String(filteredLng, 6) +
//                                "&Speed=" + gps.speed.kmph();
//             HTTPClient http;
//             String fullURL = PORT_URL + portQuery;
//             http.begin(fullURL);
...
//                     Serial.println("Datos de puerto enviados exitosamente: " + http.getString());
//                 } else {
//                     Serial.print("Error en la solicitud HTTP de puerto: ");
//                     Serial.println(httpCode);
//                     Serial.println("Respuesta: " + http.getString());
//                 }
//             } else {
//                 Serial.print("Error en la conexión HTTP de puerto: ");
